...
createArbalistMAE <- function(
  ...
  if (length(fragment.files) != length(sample.names)) {
    ...
    stop(
      'filtered.feature.matrix.files and sample names need to be the same length.'
    )
  } else if (
    !is.null(barcode.annotation.files) &&
      length(barcode.annotation.files) != length(sample.names)
  ) {
    stop(
      'barcode.annotation.files and sample names need to be the same length.'
    )
  }
  ...
  if (!is.null(barcode.anno)) {
    for (i in intersect(
      names(mae),
      c('TileMatrix500', 'GeneScoreMatrix', 'GeneAccessibilityMatrix')
    )) {
      if (any(grep('atac_', colnames(barcode.anno)))) {
        barcode.anno.atac.col <- barcode.anno[
          rownames(colData(mae[[i]])),
          setdiff(
        ...
        colnames(barcode.anno.atac.col) <- sub(
          'atac_',
          '',
          colnames(barcode.anno.atac.col)
        )
        colData(mae[[i]]) <- cbind(colData(mae[[i]]), barcode.anno.atac.col)
      } else {
        if (
          any(which(colnames(barcode.anno) == 'passed_filters')) &&
            !any(which(colnames(barcode.anno) == 'fragments'))
        ) {
          colnames(barcode.anno)[which(
            colnames(barcode.anno) == 'passed_filters'
          )] <- 'fragments'
        }
        colData(mae[[i]]) <- cbind(
          colData(mae[[i]]),
          barcode.anno[rownames(colData(mae[[i]])), ]
        )
      }
    }
    for (i in intersect(names(mae), c('GeneExpressionMatrix'))) {
      barcode.anno.gex.col <- barcode.anno[
        rownames(colData(mae[[i]])),
        grep('gex_', colnames(barcode.anno))
      ]
      colnames(barcode.anno.gex.col) <- sub(
        'gex_',
        '',
        colnames(barcode.anno.gex.col)
      )
  ...
  if (!is.null(summary.info.all)) {
    colData(mae) <- cbind(
      colData(mae),
      summary.info.all[, !colnames(summary.info.all) %in% 'Sample.ID']
    )
  }
