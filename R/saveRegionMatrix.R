...
#'
#' Count the number of fragment start/end positions within user-defined genomic
...
#'
#' If a fragment start/end overlaps with multiple entries of \code{regions}, it
#' is not counted as the assignment is ambiguous. This should not be confused
#' with overlaps to multiple intervals of the same entry when \code{regions} is
#' a GRangesList; this yields a mapping to a single feature, even if the exact
#' interval isnot  ambiguous.
#'
#' If the start and end for the same fragment overlap different entries of
#' \code{regions}, the counts for both entries are incremented by 1. This
...
#' same fragment overlap the same entry of \code{regions}, the entry's count is
#' only incremented by 1. This ensures that the count for each entry of
#' \code{regions} still follows Poisson noise and avoids an artificial
