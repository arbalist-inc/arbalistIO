...
#'
#' Count the number of fragment start/end positions within genomic tiles for
#' each cell, using the fragment file generated by Cellranger. Then, save the
#' resulting matrix into a HDF5 file.
#'
#' @param fragment.file String containing a path to a fragment file.
#' @param output.file String containing a path to an output HDF5 file. If none
...
#'   file).
#' @param barcodes Character vector of cell barcodes to extract, e.g., based on
#'   the filtered cells reported by Cellranger. If \code{NULL}, all barcodes are
...
#'   pairs.
#' @param chunk.dim Integer scalar specifying the size of the chunks (in terms
#'   of the number of elements) inside the HDF5 file.
#' @param compress.level Integer scalar specifying the Zlib compression level to
#'   use.
#'
#' @details We count the overlap with the start/end positions of each fragment,
...
#' fragment interval has no real biological significance.
#'
#' If the start and end for the same fragment overlap different tiles, the
#' counts for both tiles are incremented by 1. This reflects the fact that these
#' positions represent distinct transposase cleavage events for different
#' features. However, if the start and end for the same fragment overlap the
#' same tile, the tile's count is only incremented by 1. This ensures that the
#' count for each entry of \code{regions} still follows Poisson noise and avoids
#' an artificial enrichment of even counts.
#'
...
saveTileMatrix <- function(
  ...
  tiles <- GRanges(
    seqs,
    IRanges(starts, ends),
    seqinfo = Seqinfo(names(seq.lengths), seq.lengths)
  )
