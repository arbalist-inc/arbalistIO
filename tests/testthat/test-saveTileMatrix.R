...
test_that("createTileSCE counts fragments correctly", {
  # example 1: check for multiple fragments, multiple cells and multiple counts
  ...
  atac_sce <- createTileSCE(temp, 
                            sample.names="example3", 
                            seq.lengths=seq.lengths,
                            BPPARAM = BiocParallel::SerialParam())
  
  expected_counts <- matrix(as.raw(c(2,0,1,1,0,0)), ncol=2)
  colnames(expected_counts) <- c("example3#cell1","example3#cell2")
